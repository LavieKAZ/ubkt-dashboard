(function(){
  ...
  const FALLBACK_UNITS=['Đảng ủy phường','UBND phường','UBKT Đảng ủy','Ban Xây dựng Đảng','Văn phòng Đảng ủy','MTTQ phường','HĐND phường','Công an phường','BCHQS phường','Tất cả đơn vị'];
  ...
  function renderIndicatorRow(i){
    ...
    return `<div class="nq-row ${isChild?'child':''}">
      ...
      <div><div class="nq-body-text">${units.length?units.slice(0,3).map(roleBadge).join(' '):'<span class="nq-muted">Chưa phân công</span>'}</div></div>
  ...
  function renderUnitsView(){
    ...
    STATE.assignments.forEach(a=>{
      const key=a.unit_name||'Chưa phân công';
  ...
  async function saveIndicator(e,indId=''){
    ...
    if(primary) unitRows.push({id:id('iu'),indicator_id:newId,unit_name:primary,role:fd.get('primary_role')||'Chủ trì',is_primary:true});
