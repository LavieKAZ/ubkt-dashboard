(function(){
  ...
  const FALLBACK_UNITS=['Äáº£ng á»§y phÆ°á»ng','UBND PhÆ°á»ng','UBKT','Ban XÃ¢y dá»±ng Äáº£ng','VPÄU','MTTQ','HÄND','Äáº£ng á»§y - BCH CÃ´ng an phÆ°á»ng','BCHQS','Táº¥t cáº£ Ä‘Æ¡n vá»‹'];
  const UNIT_ALIASES=[
    ['UBND PhÆ°á»ng',['UBND','UBND phÆ°á»ng','UBND PhÆ°á»ng','á»¦y ban nhÃ¢n dÃ¢n','á»¦y ban nhÃ¢n dÃ¢n phÆ°á»ng','UBND P','uy ban nhan dan phuong']],
    ['Äáº£ng á»§y phÆ°á»ng',['Äáº£ng á»§y','Äáº£ng uá»·','Äáº£ng á»§y phÆ°á»ng','Äáº£ng uá»· phÆ°á»ng','Dang uy phuong']],
    ['Äáº£ng á»§y - BCH CÃ´ng an phÆ°á»ng',['CÃ´ng an','CÃ´ng an phÆ°á»ng','Äáº£ng á»§y CÃ´ng an phÆ°á»ng','Äáº£ng á»§y - BCH CÃ´ng an phÆ°á»ng','BCH CÃ´ng an phÆ°á»ng','CA phÆ°á»ng']],
    ['Ban XÃ¢y dá»±ng Äáº£ng',['Ban XÃ¢y dá»±ng Äáº£ng','Ban XDD','Ban XDÄ','BXDÄ','XÃ¢y dá»±ng Äáº£ng']],
    ['VPÄU',['VPÄU','VÄƒn phÃ²ng Äáº£ng á»§y','VÄƒn phÃ²ng ÄU','VP Äáº£ng á»§y','Van phong Dang uy']],
    ['UBKT',['UBKT','UBKT Äáº£ng á»§y','á»¦y ban Kiá»ƒm tra','á»¦y ban Kiá»ƒm tra Äáº£ng á»§y']],
    ['MTTQ',['MTTQ','MTTQ phÆ°á»ng','UBMTTQ','UB.MTTQ','á»¦y ban MTTQ','á»¦y ban MTTQ Viá»‡t Nam phÆ°á»ng','Máº·t tráº­n Tá»• quá»‘c']],
    ['HÄND',['HÄND','HÄND phÆ°á»ng','Há»™i Ä‘á»“ng nhÃ¢n dÃ¢n','ThÆ°á»ng trá»±c HÄND']],
    ['BCHQS',['BCHQS','BCHQS phÆ°á»ng','Ban Chá»‰ huy QuÃ¢n sá»±','Ban CHQS','QuÃ¢n sá»± phÆ°á»ng']],
    ['Táº¥t cáº£ Ä‘Æ¡n vá»‹',['Táº¥t cáº£ Ä‘Æ¡n vá»‹','Táº¥t cáº£','ToÃ n bá»™ Ä‘Æ¡n vá»‹','Táº¥t cáº£ Ä‘Æ¡n vá»‹ thá»±c hiá»‡n']],
    ['ChÆ°a phÃ¢n cÃ´ng',['','ChÆ°a xÃ¡c Ä‘á»‹nh','ChÆ°a xÃ¡c Ä‘á»‹nh Ä‘Æ¡n vá»‹','ChÆ°a phÃ¢n cÃ´ng','ChÆ°a giao']]
  ];
  ...
  function uniqueUnits(arr){return unique(arr.map(normalizeUnitName).filter(u=>u && u!=='ChÆ°a phÃ¢n cÃ´ng'));}
  ...
  function assignmentsOf(indicatorId){
    ...
    STATE.assignments.filter(a=>a.indicator_id===indicatorId).forEach(a=>{
      ...
      const key=`${unit}__${a.role||'Thá»±c hiá»‡n'}__${!!a.is_primary}`;
    ...
    return `<div class="nq-doc-node${cls}" style="margin-left:${Math.min(level*10,30)}px">
      <div class="nq-doc-head">
        ...
        <div class="nq-actions">
          <button class="nq-btn nq-btn-sm nq-btn-dark" onclick="NQIndicators.openDocumentIndicators('${d.id}')">Xem chá»‰ tiÃªu</button>
  ...
  function renderUnitsView(){
    ...
    STATE.assignments.forEach(a=>{
      const key=normalizeUnitName(a.unit_name)||'ChÆ°a phÃ¢n cÃ´ng';
  ...
  async function saveIndicator(e,indId=''){
    ...
    if(primary && primary!=='ChÆ°a phÃ¢n cÃ´ng') unitRows.push({id:id('iu'),indicator_id:newId,unit_name:primary,role:fd.get('primary_role')||'Chá»§ trÃ¬',is_primary:true});
  ...
  function renderDocIndicatorCard(i){
    ...
    return `<div class="nq-doc-ind-card">
      <button class="nq-doc-ind-main" onclick="NQIndicators.openDetail('${i.id}')">
        ...
        <span class="nq-doc-ind-meta">STT ${esc(i.ordinal??'â€”')} â€¢ ${esc(i.group_name||'ChÆ°a phÃ¢n nhÃ³m')} â€¢ ${esc(targetLabel(i))}</span>
        <span class="nq-doc-ind-units">${units.length?units.slice(0,4).map(roleBadge).join(' '):'<span class="nq-muted">ChÆ°a phÃ¢n cÃ´ng Ä‘Æ¡n vá»‹</span>'}</span>
      </button>
      <div class="nq-doc-ind-side">${badge(i.status)}${children.length?`<span class="nq-badge nq-badge-soft">${children.length} chá»‰ tiÃªu con</span>`:''}</div>
  ...
  async function openDocumentIndicators(docId){
    ...
    const body=`<div class="nq-detail-grid">
      <div class="nq-detail-box"><strong>VÄƒn báº£n</strong><p>${esc(d.code||'ChÆ°a cÃ³ sá»‘')} - ${esc(d.title)}</p></div>
      <div class="nq-detail-box"><strong>PhÃ¢n cÃ´ng theo dÃµi</strong><p>Tham mÆ°u: <b>${esc(normalizeUnitName(d.advisory_unit||'ChÆ°a phÃ¢n cÃ´ng'))}</b><br>Theo dÃµi: <b>${esc(normalizeUnitName(d.lead_unit||'ChÆ°a phÃ¢n cÃ´ng'))}</b></p></div>
    </div>
    ${childDocs.length?`<div class="nq-detail-box"><strong>VÄƒn báº£n con</strong><div class="nq-doc-child-docs">${childDocs.map(c=>`<button onclick="NQIndicators.openDocumentIndicators('${c.id}')"><b>${esc(c.code||'VB')}</b><span>${esc(c.title)}</span></button>`).join('')}</div></div>`:''}
    <div class="nq-detail-box"><strong>Danh sÃ¡ch chá»‰ tiÃªu cá»§a vÄƒn báº£n</strong><div class="nq-doc-ind-list">${list.length?list.map(renderDocIndicatorCard).join(''):'<div class="nq-empty"><b>VÄƒn báº£n nÃ y chÆ°a cÃ³ chá»‰ tiÃªu</b> Báº¥m â€œ+ Chá»‰ tiÃªuâ€ Ä‘á»ƒ nháº­p chá»‰ tiÃªu cho vÄƒn báº£n nÃ y.</div>'}</div></div>
    <div class="nq-actions"><button class="nq-btn nq-btn-primary" onclick="NQIndicators.openIndicatorForm(null,'${d.id}')">+ Chá»‰ tiÃªu cho vÄƒn báº£n nÃ y</button><button class="nq-btn nq-btn-soft" onclick="NQIndicators.openDocumentForm('${d.id}')">Sá»­a vÄƒn báº£n</button></div>`;
    openDrawer('Chá»‰ tiÃªu trong vÄƒn báº£n',`${d.code||''} ${d.title||''}`,body);
  }
