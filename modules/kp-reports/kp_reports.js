(function(){
  ...
  function renderPublic(root){const openPeriods=state.periods.filter(p=>p.status==='Đang mở'); root.innerHTML=`<div class="kp-public"><div class="kp-public-shell"><div class="kp-public-title"><h1>Nhập nhanh số liệu KTGS khu phố</h1><p>Số liệu điện tử dùng để UBKT Đảng ủy phường tổng hợp nhanh; báo cáo giấy vẫn gửi theo quy định.</p></div><form id="kpPublicForm" class="kp-panel" onsubmit="event.preventDefault();KPReports.submitPublic()"><div class="kp-panel-body"><div class="kp-section-title"><span>1</span>Thông tin đơn vị</div><div class="kp-form-grid"><label class="kp-field">Kỳ báo cáo<select id="pubPeriod" class="kp-select" required>${openPeriods.map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join('')}</select></label><label class="kp-field">Chi bộ Khu phố<select id="pubOrg" class="kp-select" required>${state.orgs.map(o=>`<option value="${esc(o.id)}">${esc(orgName(o))}</option>`).join('')}</select></label><label class="kp-field">Người nhập<input id="pubSubmitter" class="kp-input" placeholder="Họ tên"></label><label class="kp-field">Số điện thoại liên hệ<input id="pubPhone" class="kp-input" inputmode="tel" placeholder="Không bắt buộc"></label></div>${publicFields()}<div class="kp-sticky-submit"><button class="kp-btn soft" type="reset">Nhập lại</button><button class="kp-btn primary" type="submit">Gửi số liệu</button></div><div id="pubMsg" style="margin-top:12px"></div></div></form></div></div>`; if(!openPeriods.length)document.getElementById('pubMsg').innerHTML='<div class="kp-error">Hiện chưa có kỳ báo cáo đang mở. Vui lòng liên hệ UBKT Đảng ủy phường.</div>'}
  function numField(id,label){return `<label class="kp-field">${label}<input id="${id}" class="kp-input big" type="number" min="0" value="0" inputmode="numeric"></label>`}
  function publicFields(){return `<div class="kp-section-title"><span>2</span>Tình hình tổ chức đảng</div><div class="kp-form-grid three">${numField('pmTotal','Tổng số đảng viên')}${numField('pmOfficial','Đảng viên chính thức')}${numField('pmProb','Đảng viên dự bị')}${numField('pmExempt','Miễn sinh hoạt Đảng')}${numField('pmTempIn','Sinh hoạt tạm thời')}${numField('pmTempOut','Chuyển sinh hoạt tạm thời đi')}</div><div class="kp-section-title"><span>3</span>Kiểm tra, giám sát, kỷ luật</div><div class="kp-form-grid three">${numField('cvMem','KT ĐV khi có DHVP')}${numField('cvOrg','KT tổ chức khi có DHVP')}${numField('ccMem','KT ĐV chấp hành')}${numField('ccOrg','KT tổ chức chấp hành')}${numField('cdMem','KT thi hành KL đối với ĐV')}${numField('cdOrg','KT thi hành KL đối với TCĐ')}${numField('supMem','Giám sát đảng viên')}${numField('supOrg','Giám sát tổ chức đảng')}${numField('disMem','Đảng viên bị kỷ luật')}${numField('disOrg','Tổ chức đảng bị kỷ luật')}</div><div class="kp-section-title"><span>4</span>Nội dung ngắn</div><div class="kp-form-grid"><label class="kp-field">Nội dung giám sát đảng viên<textarea id="supMemText" class="kp-textarea" rows="3" placeholder="Nếu có"></textarea></label><label class="kp-field">Nội dung giám sát tổ chức đảng<textarea id="supOrgText" class="kp-textarea" rows="3" placeholder="Nếu có"></textarea></label><label class="kp-field">Giải quyết tố cáo của đảng viên<textarea id="complaint" class="kp-textarea" rows="3" placeholder="Nếu không có thì để trống"></textarea></label><label class="kp-field">Nhiệm vụ trọng tâm tiếp theo<textarea id="nextTasks" class="kp-textarea" rows="3"></textarea></label><label class="kp-field">Ghi chú/kiến nghị<textarea id="recommendations" class="kp-textarea" rows="3"></textarea></label></div>`}
