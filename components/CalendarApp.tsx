      ...
      <section className="w-full max-w-6xl h-[85vh] min-h-[650px] bg-white/40 backdrop-blur-[30px] rounded-[24px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15),inset_0_0_0_1px_rgba(255,255,255,0.5)] flex flex-col overflow-hidden relative z-10">
        <header className="h-[65px] border-b border-white/30 flex items-center justify-between px-6 bg-white/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href={GOOGLE_CALENDAR_SRC}
              target="_blank"
              rel="noreferrer"
              className="hidden sm:flex bg-white/40 border border-white/50 rounded-lg px-3 py-1.5 text-[13px] font-medium text-gray-700 items-center gap-1"
            >
              <ExternalLink className="w-4 h-4" /> Mở Google Calendar
            </a>
            <button onClick={() => openModal()} className="bg-gradient-to-r from-[#4A90E2] to-[#9013FE] text-white px-3 py-1.5 rounded-lg flex items-center text-[13px] font-medium">
              <Plus className="w-4 h-4 mr-1" /> Thêm lịch
            </button>
            <form action="/auth/signout" method="post">
              <button aria-label="Đăng xuất" className="bg-white/40 border border-white/50 rounded-lg p-1.5">
        ...
        <div className="flex-1 min-h-0 bg-white/20 p-3 md:p-4">
          <div className="h-full overflow-hidden rounded-2xl border border-white/70 bg-white/70 shadow-inner">
            <iframe
              title="Google Calendar lịch công tác"
          ...
          {events.length ? (
            <div className="mt-2 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {events.slice(-8).reverse().map((event) => (
                <button key={event.id} type="button" onClick={() => openModal(event.date)} className="min-w-[220px] rounded-xl border border-white/70 bg-white/65 px-3 py-2 text-left shadow-sm">
                  <div className="truncate text-[12px] font-bold text-gray-800">{event.title}</div>
                  <div className="mt-1 text-[11px] text-gray-500">{event.date} · {event.time.slice(0, 5)}</div>
                </button>
              ))}
            </div>
          ) : null}
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-900/10 backdrop-blur-[4px]">
            <div className="bg-white/85 backdrop-blur-3xl border border-white rounded-3xl shadow-xl w-[440px] overflow-hidden">
              ...
              <div className="px-5 py-4 border-t flex justify-end gap-3 bg-gray-50/50">
                ...
                <button onClick={saveEvent} disabled={!formData.title.trim() || isSubmitting} className="px-4 py-2 rounded-xl flex items-center text-[13px] font-medium bg-gradient-to-r from-[#4A90E2] to-[#9013FE] text-white disabled:bg-gray-200 disabled:text-gray-400">
                  {isSubmitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Đang xử lý...</> : <><Mail className="w-4 h-4 mr-2" /> Lưu lịch</>}
                </button>
              </div>
            </div>
          </div>
        ...
        <aside className={`absolute top-0 bottom-0 right-0 w-[340px] bg-white/70 backdrop-blur-3xl border-l border-white/60 shadow-xl transform transition-transform z-40 flex flex-col ${isAssistantOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="h-[65px] flex items-center justify-between px-5 border-b border-white/40">
            <span className="text-[14px] font-bold text-gray-800 flex items-center"><Sparkles className="w-5 h-5 mr-2 text-[#9013FE]" /> Trợ lý Cơ quan</span>
            <button onClick={() => setIsAssistantOpen(false)}><ChevronRight className="w-4 h-4" /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-[13px] ${message.role === "user" ? "bg-gradient-to-br from-[#4A90E2] to-[#9013FE] text-white" : "bg-white/70 text-gray-800"}`}>{message.text}</div>
              </div>
            ))}
            {isChatLoading ? <div className="pl-4 text-xs text-gray-500">Đang phân tích...</div> : null}
            <div ref={messagesEndRef} />
          </div>
          <form onSubmit={askAssistant} className="p-4 border-t border-white/40 relative">
            <input value={inputValue} onChange={(event) => setInputValue(event.target.value)} placeholder="Ra lệnh tạo lịch..." className="w-full bg-white/60 border border-white rounded-full pl-4 pr-10 py-2.5 text-[13px]" />
            <button type="submit" className="absolute right-5 top-5 p-1.5 rounded-full bg-gradient-to-r from-[#4A90E2] to-[#9013FE] text-white"><Send className="w-3.5 h-3.5" /></button>
          </form>
        </aside>
      </section>

      <button onClick={() => setIsAssistantOpen(!isAssistantOpen)} className="absolute bottom-6 right-6 z-50 p-3 rounded-full shadow-lg border border-white/80 bg-white/80 backdrop-blur-xl hover:scale-110 transition">
        {isAssistantOpen ? <X className="w-5 h-5" /> : <Sparkles className="w-6 h-6 text-[#9013FE]" />}
      </button>
    </main>
  );
}
